...
def api():
    ...
    api_key = os.environ.get('OPENAI_API_KEY') or config.get('API_KEYS', 'OPENAI_API_KEY', fallback=None)
    if not api_key:
        return jsonify({'error': 'OPENAI_API_KEY not set in environment or config file'}), 500
